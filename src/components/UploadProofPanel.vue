...
export default {
  ...
  methods: {
    ...
    lockPageScroll() {
      ...
      const pageScroller = document.querySelector('.route-page-frame')
      ...
      this.originalPageScrollStyle = {
        ...
        pageScrollerOverflow: pageScroller?.style.overflow || '',
        pageScrollerTouchAction: pageScroller?.style.touchAction || ''
      }
      ...
      // 根页面固定后滚动职责位于内容区，上传面板打开时也要同步锁住该容器。
      if (pageScroller) {
        pageScroller.style.overflow = 'hidden'
        pageScroller.style.touchAction = 'none'
      }
    },
    unlockPageScroll() {
      if (typeof window === 'undefined' || typeof document === 'undefined' || !this.originalPageScrollStyle) {
      ...
      if (pageScroller) {
        pageScroller.style.overflow = this.originalPageScrollStyle.pageScrollerOverflow
        pageScroller.style.touchAction = this.originalPageScrollStyle.pageScrollerTouchAction
      }
...
.proof-note textarea {
  ...
  color: #17211b;
  font: inherit;
  /* iOS 与部分移动 WebView 会自动放大不足 16px 的输入控件，保持 16px 可避免失焦后手动缩放。 */
  font-size: 16px;
  line-height: 1.45;
  outline: none;
  resize: none;
