...
项目、排行、历史和商城按照底部导航的左右位置播放整页滑动过渡；进入右侧页面时内容向左推进，返回左侧页面时方向反转。底部导航以带轻微流动高光的玻璃框标记当前入口，切换时玻璃框会连续滑动到目标位置。详情页面沿用所属模块的层级顺序，系统开启“减少动态效果”时自动取消横向位移和流动动画。
