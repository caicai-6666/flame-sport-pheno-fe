...
- 底部导航使用与四个主入口位置对齐的玻璃选中框。路由变化时选中框连续滑动到目标入口，框内与底栏背景使用低强度 CSS 流动高光；系统开启“减少动态效果”时停止背景流动并取消滑动时长。
- 全屏布局将 `html`、`body`、`#app` 和应用外壳固定在动态视口内，仅允许中间内容区纵向滚动。顶部 Header 与底部导航分别吸收设备安全区，内容区禁止滚动链和根页面回弹，避免上下边缘透出空白或带动固定导航位移。
