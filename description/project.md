...
- 应用启动时先显示纯前端绘制的品牌封面：明亮的蓝粉流动渐变背景、公司 Logo 和两行艺术字标语“让运动成为一种习惯，让挑战成为一种乐趣”。标语使用本地裁剪的 `src/assets/fonts/LXGWWenKai-Launch.woff2`，不依赖设备字体或网络字体；字体就绪后才开始计算最少 1.5 秒的停留时间，避免出现普通字体闪现后再跳变。登录成功后路由页会在封面下方创建并加载赛季、项目等首屏数据，封面随后淡出；弱网字体加载超时则固定使用本地系统衬线回退，封面不会永久停留。
