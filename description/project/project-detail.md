...
本文说明项目详情页的数据范围、挑战规则读取、项目锁定流程和赛季状态限制。
...
- 根据 `reward` 升序展示等级
- 请求进行中展示与卡片同尺寸的加载占位，避免规则区域留白；数据返回后，挑战卡片按列表顺序每隔约 90 ms 从右侧滑入。
- 系统开启“减少动态效果”时，加载与滑入动画应降级为静态展示。

### 异常处理

请求失败时保留项目基础信息和现有重试入口，不展示模拟挑战规则。

---

## `POST /project/lock` 锁定项目
...
当前赛季处于配置保护期时，项目介绍和全部挑战规则继续展示；锁定按钮显示“即将开始”并保持禁用，辅助文案展示距离赛季正式开始的剩余小时数。前端不发起 `POST /project/lock`，后端仍对构造请求执行最终强校验。
