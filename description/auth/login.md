...
启动封面由 CSS 流动渐变、公司 Logo 与两行艺术字标语组成，不再依赖人物海报。标语字体使用本地裁剪的 `src/assets/fonts/LXGWWenKai-Launch.woff2`；字体完整就绪后才开始计算最少 1.5 秒，避免普通字体先出现、再跳变为艺术字。`main.js` 在封面挂载后立即调用 `initLogin()`；登录成功即创建项目首页并发起赛季、项目等数据请求，这些请求会在封面停留期间并发执行。若字体在 2 秒内仍未就绪，则固定使用系统衬线回退继续流程，封面不会因资源异常永久停留。封面达到最少展示时间且登录流程结束后淡出，失败时则淡出至原有登录错误提示。
