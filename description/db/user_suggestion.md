...
| `user_id` | `VARCHAR(64)` | 非空，外键，关联 `user.id` | 提交该建议的用户 ID。 |
...
CREATE TABLE `user_suggestion` (
    ...
    `user_id` VARCHAR(64) NOT NULL COMMENT '提交建议的用户ID，关联user表主键',
...
> `user_id` 必须与 `user.id` 保持完全一致。本项目中 `user.id` 为 `VARCHAR(64)`，因此本表使用相同类型以保证外键可以创建。
